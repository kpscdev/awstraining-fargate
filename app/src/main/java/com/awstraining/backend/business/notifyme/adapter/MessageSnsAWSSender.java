...
public class MessageSnsAWSSender implements MessageSender {
    ...
    public MessageSnsAWSSender(
            AmazonSNS amazonSNS,
            @Value("${notification.topicarn}") String topicArn) {
    ...
    public void send(String text) {
        PublishRequest publishRequest = new PublishRequest(topicArn, "Hello World");
        PublishResult publishResult = amazonSNS.publish(publishRequest);
        LOGGER.info("Message send to the topic {} with {}",
                topicArn,
